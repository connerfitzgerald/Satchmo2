...
function loadMain2(parms) {
	...
	if (search == null) { search = ""; }else { search = escape(search); }
	...
	if (highlight == null) {highlight = "";} else {highlight = escape(highlight);}
		
	var poweredBy = parms.poweredBy;
	if (poweredBy == null) {poweredBy = "false";}
		
	var logo = parms.logo;
	if (logo == null) {logo = "";}
	
	var offline = parms.offline;
	if (offline == null) {offline = "false";}
	
	var hiliteColor = parms.highlightColor;
	if (hiliteColor == null) {hiliteColor = 0xFFFF00;}
	...
	var bookSrc = swf + "?opf=" + encodeURI(opf) 
			  ...
			  + "&htmlurl=" + encodeURI(document.URL)
			  + "&highlight=" + encodeURI(highlight)
			  + "&poweredBy=" + encodeURI(poweredBy)
			  + "&offline=" + encodeURI(offline)
			  + "&logo=" + encodeURI(logo)
			  + "&hiliteColor=" + encodeURI(hiliteColor)
			  + "&langs=" + encodeURI(langs)
			  + "&pages=" + encodeURI(pages);
	//bookSrc = encodeURI(bookSrc);
