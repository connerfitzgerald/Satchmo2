...
import operator
import signals
from pricingProfiles.models import *
...
class Cart(models.Model):
    ...
    def _get_total(self, include_discount=True):
        total = Decimal(0);
        for item in self.cartitem_set.all():
            total = total +Decimal(str(item.duration.price))
        return total
...
#    def add_item(self, chosen_item, number_added, details=[]):
    def add_item(self, chosen_item, duration):
        alreadyInCart = False
        # Custom Products will not be added, they will each get their own line item

        item_to_modify = CartItem(cart=self, product=chosen_item, duration=duration)
...
class CartItem(models.Model):
    ...
    duration = models.ForeignKey(PricingDate, verbose_name=_('Pricing List'))
    ...
    def __unicode__(self):
        ...
        return u'%s - %s %s' % (self.duration.price, self.product.name,
            money_format)
...
class Order(models.Model):
    ...
    def _balance(self):
        ...
        return trunc_decimal(self.total, 2)
    ...
    def force_recalculate_total(self, save=True):
        ...
        item_sub_total = Decimal("0.0000000000")
        for lineitem in self.orderitem_set.all():
            lid = lineitem.id
            print lid
#        print self.orderitem_set.all()
        for lineitem in self.orderitem_set.all():
            item_sub_total = item_sub_total + Decimal(str(lineitem.duration.price))
            #print "Run through"
            #print Decimal(lineitem.duration.price)
        self.total = Decimal(item_sub_total)
